...
class CameraViewController: UIViewController {
    ...
    var source: MediaSource? {
        didSet {
            if let oldSource = oldValue, oldSource != source {
                toggleSource()
            }
        }
    }
    ...
    private func toggleControls(isHidden: Bool) {
        ...
        if source == .capture {
            ...
            bottomBg.backgroundColor = UIColor(white: 0, alpha: 0.6)
        }
    }
    ...
    private func configureSession() {
        ...
        if mode.sources.contains(.recording) {
    ...
    private func toggleSource() {
        sessionQueue.async { [weak self] in
            guard let self = self,
                let videoDevice = self.videoDeviceInput?.device else {
                return
            }
            
            self.session.beginConfiguration()
            
            self.configSessionPreset(for: videoDevice)
            self.configRecordingFPS(for: videoDevice)
