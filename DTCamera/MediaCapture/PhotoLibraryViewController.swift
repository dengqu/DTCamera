        ...
        PHPhotoLibrary.requestAuthorization { [weak self] status in
            ...
            default:
                DispatchQueue.main.async {
                    DTMessageBar.error(message: "相册未授权")
                }
            }
        }
    }
    ...
    private func exportVideo(_ video: URL, with session: AVAssetExportSession?) {
        guard let session = session else {
            DispatchQueue.main.async {
                ...
                DTMessageBar.error(message: "获取视频导出会话失败")
            }
            return
        }
        ...
        session.exportAsynchronously { [weak self] in
            DispatchQueue.main.async { [weak self] in
                ...
                default:
                    DTMessageHUD.dismiss()
                    DTMessageBar.error(message: "视频文件导出失败")
                }
            }
        }
    }
    ...
    @objc private func done() {
        ...
        fetchPhotos { [weak self] photos in
            if let photos = photos, let mediaVC = self?.parent as? MediaViewController {
                ...
                DTMessageHUD.dismiss()
                DTMessageBar.error(message: "获取相册照片失败")
            }
        }
    }
        ...
        if asset.mediaType == .image {
            ...
            guard let videoFile = MediaViewController.videoFile,
                let imageManager = imageManager else {
                    DTMessageHUD.dismiss()
                    DTMessageBar.error(message: "创建视频文件失败")
                    return
            }
